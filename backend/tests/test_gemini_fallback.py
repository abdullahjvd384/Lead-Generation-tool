"""Verify that with no GEMINI_API_KEY set, every Gemini-powered code path
falls back cleanly to the rule-based logic.
"""
from app.services import gemini, outreach_llm, scorer_llm


def test_gemini_disabled_when_no_key(monkeypatch):
    monkeypatch.setattr("app.services.gemini._ENABLED", False)
    assert gemini.is_enabled() is False


def test_scorer_llm_returns_none_without_key(monkeypatch):
    monkeypatch.setattr("app.services.scorer_llm.gemini.is_enabled", lambda: False)
    out = scorer_llm.score_lead_llm({}, {}, {})
    assert out is None


def test_outreach_llm_returns_none_without_key(monkeypatch):
    monkeypatch.setattr("app.services.outreach_llm.gemini.is_enabled", lambda: False)
    out = outreach_llm.generate_email_llm({}, {}, {}, {})
    assert out is None


def test_scorer_llm_validation_handles_garbage(monkeypatch):
    monkeypatch.setattr("app.services.scorer_llm.gemini.is_enabled", lambda: True)
    monkeypatch.setattr(
        "app.services.scorer_llm.gemini.generate_json", lambda *a, **k: "not a dict"
    )
...
def test_scorer_llm_backfills_missing_signals(monkeypatch):
    ...
    monkeypatch.setattr("app.services.scorer_llm.gemini.generate_json", partial_response)
